...
export class GeonameComponent implements OnInit {
  ...
  constructor(private geonameDataService: GeonameDataService,
  ...
  onSearchZipcode(){
    this.geonameService.getZipcodesFromGeoName(this.zipcode).subscribe(data=>{
  ...
  updateDistances(){
    this.geonameDataService.entities$.pipe(tap(locations=>{
